...
const replyMessage = async (instructionMessage: Array<Pick<ChatMessage, 'role' | 'content'>>, timestamp: number) => {
  const model = 'Model At Server'
  ...
  const chatMessages = [
    ...
    messages.value.filter(m => {
      if (m.type === 'error' || m.type === 'canceled')
        return false
      return m.role === 'user' || (m.role === 'assistant' && (m.model === model || (model === `${(m as any).family}/${m.model}` /* incompatible old data */)))
    }).slice(-(sessionInfo.value?.attachedMessagesCount || 1)),
  ].flat()
  messages.value.push({ id, role: "assistant", content: "", type: 'loading', startTime: timestamp, endTime: timestamp, model: model })
...
const onSend = async (data: ChatBoxFormData) => {
  ...
  replyMessage(instructionMessage, timestamp)
}
