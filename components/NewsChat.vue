...
const isUserScrolling = computed(() => {
  ...
  if (messageListEl.value) {
    const bottomOffset =
      messageListEl.value.scrollHeight - messageListEl.value.clientHeight;
...
useMutationObserver(
  messageListEl,
  useThrottleFn((e: MutationRecord[]) => {
    if (
      e.some((el) => (el.target as HTMLElement).dataset.observer === "ignore")
    ) {
      return;
    }
    ...
    await clientDB.chatHistories
      ...
      .modify({
        relevantDocs: chunk.relevant_documents.map((el) => {
          const pageContent =
            el.pageContent.slice(0, 100) +
            (el.pageContent.length > 0 ? "..." : ""); // Avoid saving large-sized content
...
const fetchStream = async (url: string, options: RequestInit) => {
  ...
  if (response.status !== 200) {
    ...
    toast.add({
      title: "Error",
      description: `${errInfo}\nPlease make sure proxy is working if it's enabled.`,
      color: "red",
    });
    const errorData = {
      role: "assistant",
      type: "error",
      content: "Oops! Response Exception",
      timestamp: Date.now(),
    } as const;
  ...
  if (response.body) {
    ...
    while (true) {
      ...
      for (const line of chunk.split("\n\n")) {
        ...
        const chatMessage = JSON.parse(line) as
          | { message: Message }
          | ResponseRelevantDocument;
          ...
          if (content) {
            ...
            if (messages.value.length > 0 && lastItem.role === "assistant") {
              ...
              if (messages.value.length >= limitHistorySize) {
                messages.value = [...messages.value, itemData].slice(
                  -limitHistorySize
                );
              } else {
...
const onSend = async (data: ChatBoxFormData) => {
  ...
  const newsText = props.news?.map((n) => n.link).join("\n");
  const id = await saveMessage({
    message: `${newsText}\n${input}`,
    model: model.value || "",
    role: "user",
  ...
  const body = JSON.stringify({
    ...
    messages: [
      ...
      messages.value.slice(
        messages.value.length - sessionInfo.value!.attachedMessagesCount
      ),
      omit(userMessage, ["id"]),
    ].flat(),
    stream: true,
...
onMounted(async () => {
  await Promise.all([loadOllamaInstructions(), loadKnowledgeBases()]).then(
    ([res1, res2]) => {
...
async function initData(sessionId?: number) {
  ...
  knowledgeBaseInfo.value = knowledgeBases.find(
    (el) => el.id === result?.knowledgeBaseId
  );
  instructionInfo.value = instructions.find(
    (el) => el.id === result?.instructionId
  );
...
<template>
  <div class="flex flex-col box-border dark:text-gray-300 -mx-4">
    <div
      class="px-4 border-b border-gray-200 dark:border-gray-700 box-border h-[57px] flex items-center"
    >
      <ChatConfigInfo
        v-if="instructionInfo"
        icon="i-iconoir-terminal"
        :title="instructionInfo.name"
        :description="instructionInfo.instruction"
      />
      <ChatConfigInfo
        v-if="knowledgeBaseInfo"
      ...
      <UTooltip
        v-if="sessionId"
        text="Modify the current session configuration"
      >
        <UButton
    ...
    <div
      ref="messageListEl"
      class="relative flex-1 overflow-x-hidden overflow-y-auto px-4"
    >
      <div
      ...
      >
        ...
        <div
          class="leading-6 text-sm flex items-center max-w-full message-content"
          :class="{
            'text-gray-400 dark:text-gray-500': message.type === 'canceled',
            'flex-row-reverse': message.role === 'user',
          }"
        >
          <div
            class="border border-primary/20 rounded-lg p-3 box-border"
            :class="[
              `${
              ...
              message.type === 'error'
                ...
                : 'bg-primary-50 dark:bg-primary-400/60',
            ]"
          >
            <div
    ...
    <div
      class="shrink-0 pt-4 px-4 border-t border-gray-200 dark:border-gray-800"
    >
      <ChatInputBox
        ref="chatInputBoxRef"
        :disabled="!model"
      ...
      >
        <div class="text-muted flex">
          <UTooltip
            v-if="sessionInfo?.model"
            text="Current Model"
            :popper="{ placement: 'top-start' }"
          >
            <div
              class="flex items-center mr-4 cursor-pointer hover:text-primary-400"
              @click="onOpenSettings"
            >
          ...
          <UTooltip
            text="Attached Message Count"
            :popper="{ placement: 'top-start' }"
          >
            <div
              class="flex items-center cursor-pointer hover:text-primary-400"
              @click="onOpenSettings"
            >
              <UIcon name="i-material-symbols-history" class="mr-1"></UIcon>
              <span class="text-sm">{{
                sessionInfo?.attachedMessagesCount
              }}</span>
            </div>
          </UTooltip>
        </div>
